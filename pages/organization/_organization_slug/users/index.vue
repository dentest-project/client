<template>
  <v-main>
    ...
    <v-snackbar v-model="addConflictErrorSnackbarOpened" :color="$colors.error"
      >The user is already in this organization</v-snackbar
    >
...
export default Vue.extend({
  ...
  methods: {
    onUserAdd: async function (user: BaseUser) {
      try {
        await this.$api.createOrganizationUser(
          this.organization,
          user,
          this.$axios
        )
    ...
    onUserChanged: async function (user: OrganizationUser) {
      ...
      try {
        await this.$api.updateOrganizationUser(
          this.organization.id,
          user.user.id,
          user.permissions,
          this.$axios
    ...
    reload: async function () {
      ...
      this.users = await this.$api.getOrganizationUsers(
