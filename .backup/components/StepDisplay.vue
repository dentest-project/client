<template>
  <div v-if="step.step" class="step-display">
    <div class="step-display-sentence">
      ...
      <span v-for="part in step.step.parts">
        <span v-if="part.type === 'sentence'">
          {{ part.content }}</span
        >
        <span v-else class="step-display-inline-param">
    ...
    >
      <thead v-if="tableParamOptions.headerRow && extraParamValue.length > 0">
        <tr>
          <th v-for="head in extraParamValue[0]">
            {{ head }}
          </th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="row in tableParamOptions.headerRow ? extraParamValue.slice(1) : extraParamValue">
          <th v-if="tableParamOptions.headerColumn">
            {{ row[0] }}
          </th>
          <td v-for="cell in tableParamOptions.headerColumn ? row.slice(1) : row">
            {{ cell }}
          </td>
        </tr>
      </tbody>
