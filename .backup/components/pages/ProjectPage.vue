<template>
  <v-main>
    ...
    <actions-bar v-if="$auth.loggedIn && isProjectUser">
      <add-folder-button
        v-if="canWrite"
        @click.stop="activateCreatePathDialog"
      />
      <add-feature-button
        v-if="canWrite"
        @click.stop="activateCreateFeatureDialog"
      />
      <v-spacer />
      ...
      <visibility-button
        ...
        :visibility="path.rootProject.visibility"
        @click.stop="onVisibilityUpdated"
      />
      <exit-button
        v-if="isProjectUser"
        label="Leave project"
    ...
    <grid3>
      ...
      />
      <feature-card
        v-for="feature in path.features"
        :feature="feature"
        :path="path"
        :can-write="canWrite"
        @moved="onFeatureMoved"
      />
    </grid3>
    <create-path-dialog
...
export default Vue.extend({
  ...
  methods: {
    ...
    async onFeatureMoved(newPath: UpdateFeatureParentPath) {
      try {
        await this.$api.updateFeaturePath(newPath)
    ...
    async onPathMoved(newPath: UpdatePathParent) {
      try {
        await this.$api.updatePathParent(newPath)
  ...
  computed: {
    title(): string {
      return (this as any).path.project
        ? (this as any).path.project.title
    ...
    canAdministrate: function (): boolean {
      ...
      return (
        typeof path.rootProject.permissions.find(
