<template>
  <v-dialog width="480" :value=value @input="onDialogStatusChanged">
    <v-card>
      ...
      <v-card-text>
        Are you sure that you want to delete this project? All its content will be deleted.
      </v-card-text>
      <v-card-actions>
        <v-btn :color="$colors.cancel" text @click.stop="cancel">Cancel</v-btn>
        <v-spacer></v-spacer>
        <v-btn :color="$colors.error" text @click.stop="confirm">Delete project</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
